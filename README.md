...
- As linhas de array passadas para `ParamBox()` (em `ZCT020.prw` e `ZCT040.prw`) têm tamanho fixo por tipo de pergunta e não são tolerantes a elementos faltando: **tipo 1** (campo digitável) exige exatamente 9 elementos — `{nTipo, cDescricao, xInicial, cPicture, cValidacao, cF3, cWhen, nTamanho, lObrigatorio}` —, **tipo 2** (combo) exige exatamente 7 — `{nTipo, cDescricao, nOpcaoInicial, aOpcoes, nTamanho, cValidacao, lObrigatorio}`. Faltar um elemento não dá erro de sintaxe, mas explode em runtime dentro do próprio `ParamBox` ("array out of bounds"), pois cada posição é lida por índice fixo internamente. Além disso, mesmo o `nOpcaoInicial` (3ª posição, ao montar a pergunta) sendo numérico, o **retorno** de uma pergunta tipo 2 em `aRetorno[]` vem sempre como a **string do texto selecionado** (ex.: `"Sim"`/`"Nao"`), nunca como índice — comparar com um número (`aRetorno[3] == 1`) gera erro em runtime ("type mismatch on compare"); por isso `ZCT020` compara `aRetorno[3] == "Sim"`.
